...
def test_config_binds_nested_fifty_image_pool_and_outputs() -> None:
    ...
    assert config.correlation.sanity_failure_threshold == pytest.approx(0.5)
    assert config.outputs.summary_table.name == "gradcam_sanity_summary.csv"
    assert config.outputs.panel_figure.name == "gradcam_sanity_panel.png"
...
def test_aggregation_reports_declared_k_and_failure_rates() -> None:
    ...
    rows = aggregate_sanity(records, sampling_audit=sampling, failure_threshold=0.5)

    assert len(rows) == 4
    assert all(row["k_valid_pairs"] == 1 for row in rows)
    assert all(row["map_pair_failure_rate"] == pytest.approx(0.5) for row in rows)
    assert all(row["c_sanity"] == pytest.approx(0.25) for row in rows)
    assert all(row["sanity_failure_rate"] == pytest.approx(0.0) for row in rows)
