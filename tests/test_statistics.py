...
def test_clean_holm_uses_all_estimable_headline_metrics() -> None:
    rows = [
        {
            "status": "complete",
            "p_value_raw": value,
    ...
    assert [row["p_value_holm"] for row in rows] == pytest.approx([0.03, 0.06, 0.06])
    assert [row["holm_family_size"] for row in rows] == [3, 3, 3]
