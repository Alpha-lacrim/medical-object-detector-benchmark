...
The controlled protocol also exposes an operating-point problem that published single-threshold comparisons can conceal. At the original shared score threshold of 0.25, YOLO11s appears more precise but operates at far lower recall. The five-run frozen-bundle sensitivity shows that Faster R-CNN has the stronger precision-recall frontier and higher sensitivity at every reported false-positive budget; the original three-seed artifacts are retained as the pre-specified historical analysis. Detector-specific thresholds selected on three-run validation evidence also differ sharply (0.69 versus 0.05), and those choices were applied unchanged to all five test bundles. Thus a shared nominal threshold does not guarantee a shared operating regime: it can select different portions of the detectors' score distributions. This is an empirical score-scale/selectivity finding of the present study, not a claim that either score is probabilistically calibrated.
