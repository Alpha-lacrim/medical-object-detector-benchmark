...
Refreshed for Batch 46's focused internal manuscript, after verification of
the scientific outputs from Batches 42--45. Current manuscript SHA-256:
`75c214ae28ceffc643f85dae96d4a58a18b172458a867bc867e69c11d0563b5e`.
The main paper now centers on operating-point definition, five retained runs,
training-procedure uncertainty, and matched implementation timing. Secondary
methods/results remain available through the unchanged supplementary index.
The title and abstract are provisional pending external evidence.
...
| 4 | A priori aims, objectives, and hypotheses | No | The aim is explicit, but the analyses and hypotheses are retrospective; historical H1--H6 labels remain only in repository traceability records; see [`paper_draft.md` §1](../report/paper_draft.md#1-introduction) and [`HYPOTHESIS_TRACEABILITY.md`](HYPOTHESIS_TRACEABILITY.md). |
| 5 | State whether the study is prospective or retrospective | Yes | The [abstract](../report/paper_draft.md#abstract) plainly identifies a retrospective internal-testing design, and [§1](../report/paper_draft.md#1-introduction) discloses retrospective analysis without preregistration. |
...
| 8 | Eligibility, setting, location, dates, demographics, and sampling method | No | Source, subset, strata, sampling, and partition-wise DICOM-header age/sex/AP-PA characteristics are reported, but source accrual dates, detailed eligibility, and broader demographic/clinical variables remain unavailable; the cohort cannot be characterized as consecutive, random clinical sampling. |
...
| 23 | Software versions and hardware | Yes | [`paper_draft.md` §§3.2 and 3.6](../report/paper_draft.md#36-standardized-implementation-timing), [`uv.lock`](../uv.lock), the [matched v1 timing summary](../results/logs/phase45_inference_timing_v1/summary.json), and the [historical evaluation summary](../results/logs/phase5_evaluation/summary.json) record versions and the measured system. Batch 45 verifies the intended reporting hardware and distinguishes technical repeats from training runs. |
...
| 26 | Final-model selection | Yes | [`paper_draft.md` §§3.2--3.3](../report/paper_draft.md#33-five-retained-runs-and-common-evaluation) reports model-optimization-split mAP selection before frozen internal testing; per-run checkpoint metadata are indexed in [`SUPPLEMENTARY.md` S2](SUPPLEMENTARY.md#s2-full-clean-seed-level-comparison). |
...
| 29 | Uncertainty/significance procedures and statistical software | Yes | [`paper_draft.md` §3.5](../report/paper_draft.md#35-training-procedure-uncertainty), [`docs/STATISTICAL_ANALYSIS.md`](STATISTICAL_ANALYSIS.md), [`results/tables/statistical_clean_comparison.csv`](../results/tables/statistical_clean_comparison.csv), and [`results/logs/phase8_statistics/summary.json`](../results/logs/phase8_statistics/summary.json) separate training-procedure intervals from checkpoint-conditional permutation tests and record seeds/draw counts. |
| 30 | Robustness or sensitivity analyses | Yes | [`paper_draft.md` §§3.4, 3.7, 4.2--4.3, and 4.6](../report/paper_draft.md#34-operating-point-protocols-and-exact-score-froc), [`docs/THRESHOLD_ANALYSIS.md`](THRESHOLD_ANALYSIS.md), and [`docs/ACQUISITION_SHIFTS.md`](ACQUISITION_SHIFTS.md) report post-hoc validation-threshold sensitivity, digital corruptions, and bounded radiography-motivated synthetic sensitivities. |
| 31 | Explainability methods, parameters, and validation/sanity checks | Yes | [`paper_draft.md` §§3.7 and 4.6](../report/paper_draft.md#37-secondary-analyses), [`docs/EXPLAINABILITY.md`](EXPLAINABILITY.md), and [`docs/XAI_SANITY.md`](XAI_SANITY.md) record layers, targets, localization measures, and randomization controls. |
...
| 36 | Demographic and clinical characteristics by partition | Yes | [`paper_draft.md` §3.1](../report/paper_draft.md#31-dataset-target-and-patient-disjoint-split) reports studies, patient groups, opacity-positive studies, boxes, age, sex, AP/PA projection, and missingness for training, validation, internal testing, and total. The [aggregate table](../results/tables/rsna_cohort_characteristics.csv) and [provenance summary](../results/logs/phase44_cohort_characteristics/summary.json) contain no raw patient rows. |
...
| 39 | Failure analysis | Yes | [`paper_draft.md` §§4.1--4.3 and 4.6](../report/paper_draft.md#41-clean-internal-testing-performance) covers seed-271 score compression, operating-regime sensitivity, corruptions, DICOM evidence boundaries, and XAI failure patterns. |
...
| Internal versus external testing | Patient-disjoint held-out evaluation is explicitly called internal testing in [`paper_draft.md` §3.3](../report/paper_draft.md#33-five-retained-runs-and-common-evaluation). | No external testing was performed. |
...
| Uncertainty, indeterminate outputs, and failures | [`paper_draft.md` §§3.5, 4.1, and 4.4](../report/paper_draft.md#35-training-procedure-uncertainty) reports intervals, the zero-detection YOLO run, and endpoint-specific undefinedness. | Multiple boxes/false positives and conditional localization do not reduce to one diagnostic result per participant. |
| Generalizability and clinical role | [`paper_draft.md` §§5--6](../report/paper_draft.md#5-discussion) explicitly limits use. | No clinical accuracy, prospective benefit, fairness, or external-site evidence exists. |
...
| Model performance and calibration | Detection metrics, uncertainty, and emitted-detection D-ECE are in [`paper_draft.md` §§3.3, 3.7, and 4](../report/paper_draft.md#33-five-retained-runs-and-common-evaluation). | D-ECE is not exam-level outcome-probability calibration; decision-curve analysis was correctly not performed. |
...
- Use [`CITATION_AUDIT.md`](CITATION_AUDIT.md) for the current 16-key
